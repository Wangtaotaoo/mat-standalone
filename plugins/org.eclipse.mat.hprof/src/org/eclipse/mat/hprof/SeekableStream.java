...
public class SeekableStream extends InputStream implements AutoCloseable
{
    ...
    static class PosStream extends FilterInputStream implements Comparable<PosStream>
    {
        ...
        boolean isCleared()
        {
            return savedStream == null || savedStream.get() == null;
        }
        ...
        public void close() throws IOException
        {
            if (setActive(true))
            {
                ...
                savedStream.clear();
                in = null;
                // Streams should be closed so no need to finalize()
            }
        }
    }
    ...
    private final boolean verbose = false; // Standalone: no Platform debug mode
    /**
