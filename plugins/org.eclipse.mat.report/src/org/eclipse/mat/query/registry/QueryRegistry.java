...
public class QueryRegistry extends RegistryReader<IQuery>
{
    ...
    {
        init(null, ReportPlugin.PLUGIN_ID + ".query"); //$NON-NLS-1$
    }

    @Override
    protected IQuery createDelegate(Object configElement)
    {
        return null;
    }
    ...
    private Object findBundle(Class<?> cls)
    {
    ...
    {
        URL icon = null;
        // Use classloader-based resource loading
        String v = iconPath;
        v = v.replace("$nl$", ""); //$NON-NLS-1$//$NON-NLS-2$
        v = v.replace("$ws$", ""); //$NON-NLS-1$//$NON-NLS-2$
        v = v.replace("$os$", ""); //$NON-NLS-1$//$NON-NLS-2$
        // Try as absolute path from classloader
        if (v.startsWith("/")) //$NON-NLS-1$
            icon = queryClass.getClassLoader().getResource(v.substring(1));
        if (icon == null)
